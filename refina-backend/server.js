// refina-backend/server.js (ESM, PROD-ONLY, Express v5-safe) - Theme App Embed fix (Gemini)
...
app.use("/proxy/refina", createProxyMiddleware({
...
app.get("/launcher.js", async (_req, res) => {
  try {
    ...
    res
      ...
      .send(`(function(){ const css = ${JSON.stringify(cssUrls)}; for (const href of css) { const l=document.createElement("link"); l.rel="stylesheet"; l.href=href; document.head.appendChild(l); } const s=document.createElement("script"); s.type="module"; s.src=${JSON.stringify(entryUrl)}; document.head.appendChild(s); })();`);
  } catch (e) {
    res
      .type("application/javascript")
      .status(500)
...
// BFF API endpoints
app.get("/v1/health", (_req, res) => res.json({ ok: true, now: new Date().toISOString(), cacheSize: cache.size, version: "unified-server" }));
app.post("/v1/recommend", express.json(), async (req, res) => {
  ...
  try {
    ...
    const hydrate = used.map((id) => {
      ...
      return { id, title: p.title || p.name || "", name: p.title || p.name || "", image: p.image || p.images?.[0]?.src || "", description: p.description || "", productType: p.productType || "", tags: p.tags || [], url: productUrl, price: p.price ?? null };
    });
