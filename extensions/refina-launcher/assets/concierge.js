...
function readRefinaPrefillOnce() {
  ...
  function initOne(root) {
    ...
    (function attachPrefillBridgeOnce() {
     ...
     document.addEventListener('refina:prefill:request', () => {
  // Use NON-destructive read here; the iframe URL build will still see it
...
for (const [datasetKey, paramName] of Object.entries(THEME_PARAM_MAP)) {
  ...
  base.searchParams.set(paramName, String(v));
    ...
    function openModal() {
      ...
      iframe.src = buildIframeUrl();
      iframe.title = "Refina concierge";
      // After the iframe loads, send the payload across the boundary, then clear it
iframe.addEventListener('load', () => {
  try {
    const saved = readRefinaPrefill();
    if (saved && iframe.contentWindow && typeof iframe.contentWindow.postMessage === 'function') {
      iframe.contentWindow.postMessage({ type: 'refina:prefill', payload: saved }, '*');
      // one-shot semantics: clear ONLY after we've delivered it
      sessionStorage.removeItem('refina_prefill');
    }
  } catch {}
});
