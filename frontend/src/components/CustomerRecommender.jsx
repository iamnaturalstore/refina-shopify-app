        ...
        <>
          ...
          <div className={styles.grid} role="list">
            {matchedProducts.map((product, idx) => {
              ...
              return (
                <div key={product.id || product.name} className={styles.card} role="listitem" onClick={() => setSelectedProduct(product)}>
                  <img src={product.image} alt={product.name} className={styles.image} onError={(e) => { e.currentTarget.src = "https://cdn.shopify.com/s/images/admin/no-image-compact.gif"; }} />
                  {isTopPick && <div className={styles.topPickBadge} aria-label="Top pick">Top pick</div>}
                  <h3 className={styles.productTitle}>{product.name}</h3>
                  <p className={styles.productDescription}>{decodeEntities(teaser)}</p>
                  {formatPrice(product.price) && <div className={styles.price}>{formatPrice(product.price)}</div>}
                </div>
              );
            })}
          </div>
      ...
      {selectedProduct && (
        <div className={styles.modalOverlay} onClick={() => setSelectedProduct(null)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <img src={selectedProduct.image} alt={selectedProduct.name} onError={(e) => { e.currentTarget.src = "https://cdn.shopify.com/s/images/admin/no-image-compact.gif"; }} style={{ marginTop: 12 }} />
            <div style={{ marginTop: 12, lineHeight: 1.5 }}>
              {reasonsById?.[selectedProduct.id] ? (<p>{reasonsById[selectedProduct.id]}</p>) : (<p>{teaserFromHtml(selectedProduct.description || "")}</p>)}
              {copy.extras ? <p style={{ opacity: 0.85 }}>{copy.extras}</p> : null}
            </div>
            <a href={selectedProduct.url || selectedProduct.link || "#"} target="_blank" rel="noopener noreferrer" className={styles.buyNow}>
              Buy Now
            </a>
          </div>
        </div>
